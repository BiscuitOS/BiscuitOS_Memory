...
#define ALLOC_NO_WATERMARKS_BS		0x01 /* don't check watermarks at all */
#define ALLOC_WMARK_MIN_BS		0x02 /* use pages_min watermark */
#define ALLOC_WMARK_LOW_BS		0x04 /* use pages_low watermark */
#define ALLOC_WMARK_HIGH_BS		0x08 /* use pages_high watermark */
#define ALLOC_HARDER_BS			0x10 /* try to alloc harder */
#define ALLOC_HIGH_BS			0x20 /* __GFP_HIGH set */
#define ALLOC_CPUSET_BS			0x40 /* check for correct cpuset */
...
#endif
struct pglist_data_bs *pgdat_list_bs __read_mostly;
 ...
 */
struct zone_bs *zone_table_bs[1 << ZONETABLE_SHIFT_BS] __read_mostly;
 ...
 */
int sysctl_lowmem_reserve_ratio_bs[MAX_NR_ZONES_BS-1] = { 256, 256, 32 };

unsigned long __initdata nr_kernel_pages_bs;
unsigned long __initdata nr_all_pages_bs;
unsigned long totalram_pages_bs __read_mostly;
unsigned long totalhigh_pages_bs __read_mostly;

static char *zone_names_bs[MAX_NR_ZONES_BS] = { 
	"DMA", 
	"DMA32", 
	"Normal", 
	"HighMem"
...
static void destroy_compound_page_bs(struct page_bs *page, unsigned long order);
...
static void __get_page_state_bs(struct page_state_bs *ret,
                                        int nr, cpumask_t *cpumask);
...
void get_page_state_bs(struct page_state_bs *ret)
{
	...
	__get_page_state_bs(ret, nr + 1, &mask);
}
 ...
 */
static void __unused setup_pagelist_highmark_bs(struct per_cpu_pageset_bs *p,
							unsigned long high)
{
	struct per_cpu_pages_bs *pcp;
	...
	if ((high/4) > (PAGE_SHIFT_BS * 8))
		pcp->batch = PAGE_SHIFT_BS * 8;
}
...
static int __devinit zone_batchsize_bs(struct zone_bs *zone)
{
	...
	if (batch * PAGE_SIZE_BS > 512 * 1024)
		batch = (512 * 1024) / PAGE_SIZE_BS;
...
static __init
void zone_wait_table_init_bs(struct zone_bs *zone, 
				unsigned long zone_size_pages)
{
	int i;
	struct pglist_data_bs *pgdat = zone->zone_pgdat;

	/*
	 * The per-page waitqueue mechanism uses hashed waitqueues
	 * per zone.
	 */
	zone->wait_table_size = wait_table_size_bs(zone_size_pages);
	zone->wait_table_bits =	wait_table_bits_bs(zone->wait_table_size);
...
static __init void zone_pcp_init_bs(struct zone_bs *zone)
{
	int cpu;
	unsigned long batch = zone_batchsize_bs(zone);

	for (cpu = 0; cpu < NR_CPUS_BS; cpu++) {
#ifdef CONFIG_NUMA
		/* Early boot. Slab allocator not functional yet */
		zone_pcp_bs(zone, cpu) = &boot_pageset_bs[cpu];
		setup_pageset(&boot_pageset_bs[cpu],0);
#else
...
static __init void init_currently_empty_zone_bs(struct zone_bs *zone,
		unsigned long zone_start_pfn, unsigned long size)
{
	struct pglist_data_bs *pgdat = zone->zone_pgdat;

	zone_wait_table_init_bs(zone, size);
	pgdat->nr_zones = zone_idx(zone) + 1;

	zone->zone_mem_map = pfn_to_page_bs(zone_start_pfn);
	zone->zone_start_pfn = zone_start_pfn;

	memmap_init_bs(size, pgdat->node_id, zone_idx_bs(zone), zone_start_pfn);
		...
		unsigned long *zones_size, unsigned long *zholes_size)
{
	...
	for (j = 0; j < MAX_NR_ZONES_BS; j++) {
		...
		init_currently_empty_zone_bs(zone, zone_start_pfn, size);
		zone_start_pfn += size;
	}
}
...
static int __init build_zonelists_node_bs(pg_data_t_bs *pgdat,
		struct zonelist_bs *zonelist, int nr_zones, int zone_type)
{
	struct zone_bs *zone;

	BUG_ON_BS(zone_type > ZONE_HIGHMEM_BS);

	do {
		zone = pgdat->node_zones + zone_type;
		if (populated_zone_bs(zone)) {
#ifndef CONFIG_HIGHMEM_BS
			BUG_ON_BS(zone_type > ZONE_NORMAL_BS);
#endif
			zonelist->zones[nr_zones++] = zone;
			check_highest_zone_bs(zone_type);
		}
		zone_type--;
...
static inline int highest_zone_bs(int zone_bits)
{
	int res = ZONE_NORMAL_BS;
	if (zone_bits & (__force int)__GFP_HIGHMEM_BS)
		res = ZONE_HIGHMEM_BS;
	if (zone_bits & (__force int)__GFP_DMA32_BS)
		res = ZONE_DMA32_BS;
	if (zone_bits & (__force int)__GFP_DMA_BS)
		res = ZONE_DMA_BS;
	return res;
}
...
static int page_outside_zone_boundaries_bs(struct zone_bs *zone, 
							struct page_bs *page)
{
	int ret = 0;
	unsigned seq;
	unsigned long pfn = page_to_pfn_bs(page);

	do {
		seq = zone_span_seqbegin_bs(zone);
		if (pfn >= zone->zone_start_pfn + zone->spanned_pages)
			ret = 1;
		else if (pfn < zone->zone_start_pfn)
			ret = 1;
	} while (zone_span_seqretry_bs(zone, seq));

	return ret;
}

static int page_is_consistent_bs(struct zone_bs *zone, struct page_bs *page)
{
...
{
	if (page_outside_zone_boundaries_bs(zone, page))
		return 1;
	if (!page_is_consistent_bs(zone, page))
		return 1;

	return 0;
}

unsigned long read_page_state_offset_bs(unsigned long offset)
{
	unsigned long ret = 0;
	int cpu;
...
void __mod_page_state_offset_bs(unsigned long offset, unsigned long delta)
{
	void *ptr;
...
EXPORT_SYMBOL(__mod_page_state_offset_bs);

void mod_page_state_offset_bs(unsigned long offset, unsigned long delta)
{
	unsigned long flags;
	void *ptr;
...
EXPORT_SYMBOL_GPL(mod_page_state_offset_bs);

static void bad_page_bs(struct page_bs *page)
{
	...
	KERN_EMERG "Backtrace:\n",
		...
		page_mapcount_bs(page), page_count_bs(page));
	...
	reset_page_mapcount_bs(page);
	page->mapping = NULL;
	add_taint_bs(TAINT_BAD_PAGE_BS);
}

static inline int free_pages_check_bs(struct page_bs *page)
{
	if (unlikely(page_mapcount_bs(page) |
		(page->mapping != NULL) |
	...
	if (PageDirty_bs(page))
		__ClearPageDirty_bs(page);
	/*
	 * For now, we report if PG_reserved was found set, but do not
	 * clear it, and do not free the page.  But we shall soon need
	 * to do more, for when the ZERO_PAGE count wraps negative.
	 */
	return PageReserved_bs(page);
}
...
static inline void __free_one_page_bs(struct page_bs *page,
                        ...
                        struct list_head *list, unsigned int order)
{
	...
	while (count--) {
		...
		BUG_ON_BS(list_empty(list));
		page = list_entry(list->prev, struct page_bs, lru);
		/* have to delete it as _free_one_page_bs list manipulates */
		list_del(&page->lru);
		__free_one_page_bs(page, zone, order);
	}
	spin_unlock(&zone->lock);
}

static void free_one_page_bs(struct zone_bs *zone, struct page_bs *page,
			int order)
{
	LIST_HEAD(list);
	list_add(&page->lru, &list);
	free_pages_bulk_bs(zone, 1, &list, order);
}
...
void __free_pages_ok_bs(struct page_bs *page, unsigned int order)
{
	...
	for (i = 0; i < (1 << order) ; ++i)
		reserved += free_pages_check_bs(page + i);
	if (reserved)
		return;

	kernel_map_pages_bs(page, 1 << order, 0);
	local_irq_save(flags);
	__mod_page_state_bs(pgfree, 1 << order);
	free_one_page_bs(page_zone_bs(page), page, order);
	local_irq_restore(flags);
}
...
static void fastcall_bs free_hot_cold_page_bs(struct page_bs *page, int cold)
{
	...
	if (PageAnon_bs(page))
		page->mapping = NULL;
	if (free_pages_check_bs(page))
		return;

	kernel_map_pages_bs(page, 1, 0);
	pcp = &zone_pcp_bs(zone, get_cpu())->pcp[cold];
	local_irq_save(flags);
	__inc_page_state_bs(pgfree);
	list_add(&page->lru, &pcp->list);
	pcp->count++;
	if (pcp->count >= pcp->high) {
		free_pages_bulk_bs(zone, pcp->batch, &pcp->list, 0);
		pcp->count -= pcp->batch;
	}
	local_irq_restore(flags);
	put_cpu();
		...
		int classzone_idx, int alloc_flags)
{
	...
	if (alloc_flags & ALLOC_HIGH_BS)
		min -= min / 2;
	if (alloc_flags & ALLOC_HARDER_BS)
		min -= min / 4;
...
static struct page_bs *__rmqueue_bs(struct zone_bs *zone, unsigned int order)
{
	...
	for (current_order = order; current_order < MAX_ORDER_BS; 
		...
		expand_bs(zone, page, order, current_order, area);
		return page;
	}
	return NULL;
}
		...
		unsigned long count, struct list_head *list)
{
	...
	spin_lock(&zone->lock);
	for (i = 0; i < count; ++i) {
		struct page_bs *page = __rmqueue_bs(zone, order);
 ...
 */
static int prep_new_page_bs(struct page_bs *page, int order)
{
	if (unlikely(page_mapcount_bs(page)	|
		(page->mapping != NULL) |
		(page_count_bs(page) != 0) |
 ...
 */
static void free_compound_page_bs(struct page_bs *page)
{
	__free_pages_ok_bs(page, (unsigned long)page[1].lru.prev);
}

static void prep_compound_page_bs(struct page_bs *page, unsigned long order)
{
	int i;
	int nr_pages = 1 << order;

	page[1].lru.next = (void *)free_compound_page_bs; /* set dtor */
	page[1].lru.prev = (void *)order;
	for (i = 0; i < nr_pages; i++) {
		struct page_bs *p = page + i;

		SetPageCompound_bs(p);
		set_page_private_bs(p, (unsigned long)page);
	}
}
...
static void destroy_compound_page_bs(struct page_bs *page, unsigned long order)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		if (unlikely(!PageCompound_bs(p) |
				(page_private_bs(p) != (unsigned long)page)))
			bad_page_bs(page);
		ClearPageCompound_bs(p);
	}
}

static void zone_statistics_bs(struct zonelist_bs *zonelist,
						struct zone_bs *z, int cpu)
{
}
 ...
 * or two.
 */
static struct page_bs * buffered_rmqueue_bs(struct zonelist_bs *zonelist,
		struct zone_bs *zone, int order, gfp_t_bs __nocast gfp_flags)
{
	unsigned long flags;
	struct page_bs *page;
...
again:
	...
	if (likely(order == 0)) {
		...
		pcp = &zone_pcp_bs(zone, cpu)->pcp[cold];
		local_irq_save(flags);
		if (!pcp->count) {
			pcp->count += rmqueue_bulk_bs(zone, 0, pcp->batch,
							&pcp->list);
	...
	__mod_page_state_zone_bs(zone, pgalloc, 1 << order);
	zone_statistics_bs(zonelist, zone, cpu);
	local_irq_restore(flags);
	put_cpu();

	BUG_ON_BS(bad_range_bs(zone, page));
	if (prep_new_page_bs(page, order))
		goto again;
...
get_page_from_freelist_bs(gfp_t_bs gfp_mask, unsigned int order,
		struct zonelist_bs *zonelist, int alloc_flags)
{
	struct zone_bs **z = zonelist->zones;
	struct page_bs *page = NULL;
	int classzone_idx = zone_idx_bs(*z);
	 ...
	 */
	do {
		if ((alloc_flags & ALLOC_CPUSET_BS) &&
				!cpuset_zone_allowed_bs(*z, gfp_mask))
			continue;

		if (!(alloc_flags & ALLOC_NO_WATERMARKS_BS)) {
			unsigned long mark;
			if (alloc_flags & ALLOC_WMARK_MIN_BS)
				mark = (*z)->pages_min;
			else if (alloc_flags & ALLOC_WMARK_LOW_BS)
				mark = (*z)->pages_low;
			else
				mark = (*z)->pages_high;
			if (!zone_watermark_ok_bs(*z, order, mark,
				    classzone_idx, alloc_flags))
				if (!zone_reclaim_mode_bs ||
				    !zone_reclaim_bs(*z, gfp_mask, order))
					continue;
		}

		page = buffered_rmqueue_bs(zonelist, *z, order, gfp_mask);
		if (page) {
			break;
		}
	} while (*(++z) != NULL);
...
__alloc_pages_bs(gfp_t_bs __nocast gfp_mask, unsigned int order,
		struct zonelist_bs *zonelist)
{
	const gfp_t wait = gfp_mask & __GFP_WAIT_BS;
	struct zone_bs **z;
	struct page_bs *page;
...
restart:
	...
	page = get_page_from_freelist_bs(gfp_mask|__GFP_HARDWALL_BS, order,
				zonelist, ALLOC_WMARK_LOW_BS|ALLOC_CPUSET_BS);
	if (page)
		goto got_pg;
	 ...
	 */
	alloc_flags = ALLOC_WMARK_MIN_BS;
	if ((unlikely(rt_task(p)) && !in_interrupt()) || !wait)
		alloc_flags |= ALLOC_HARDER_BS;
	if (gfp_mask & __GFP_HIGH_BS)
		alloc_flags |= ALLOC_HIGH_BS;
	alloc_flags |= ALLOC_CPUSET_BS;
	 ...
	 * See also cpuset_zone_allowed() comment in kernel/cpuset.c.
	 */
	page = get_page_from_freelist_bs(gfp_mask, order, 
	...
	if (((p->flags & PF_MEMALLOC_BS) || 
				unlikely(test_thread_flag(TIF_MEMDIE_BS)))
			&& !in_interrupt()) {
		if (!(gfp_mask & __GFP_NOMEMALLOC_BS)) {
nofail_alloc:
			/* go through the zonelist yet again, ignoring mins */
			page = get_page_from_freelist_bs(gfp_mask, order,
				zonelist, ALLOC_NO_WATERMARKS_BS);
			if (page)
				goto got_pg;
			if (gfp_mask & __GFP_NOFAIL_BS) {
				blk_congestion_wait_bs(WRITE_BS, HZ/50);
				goto nofail_alloc;
			}
		}
		goto nopage;
...
rebalance:
	...
	cpuset_memory_pressure_bump_bs();
	p->flags |= PF_MEMALLOC_BS;
	reclaim_state.reclaimed_slab = 0;
	p->reclaim_state = (struct reclaim_state *)&reclaim_state;

	did_some_progress = try_to_free_pages_bs(zonelist->zones, gfp_mask);
	...
	if (likely(did_some_progress)) {
		 ...
		 */
		page = get_page_from_freelist_bs(gfp_mask|__GFP_HARDWALL_BS,
			order, zonelist, ALLOC_WMARK_HIGH_BS|ALLOC_CPUSET_BS);
		if (page)
			goto got_pg;

		out_of_memory_bs(zonelist, gfp_mask, order);
		goto restart;
	}
...
fastcall_bs unsigned long get_zeroed_page_bs(gfp_t_bs __nocast gfp_mask)
{
	 ...
	 * a highmem page.
	 */
	BUG_ON_BS((gfp_mask & __GFP_HIGHMEM_BS) != 0);
 ...
 */
fastcall_bs unsigned long __get_free_pages_bs(gfp_t_bs __nocast gfp_mask,
						unsigned int order)
{
	struct page_bs *page;
...
void get_page_state_node(struct page_state_bs *ret, int node)
{
	int nr;
	cpumask_t mask = node_to_cpumask_bs(node);
...
void get_full_page_state_bs(struct page_state_bs *ret)
{
	...
	__get_page_state_bs(ret, sizeof(*ret) / sizeof(unsigned long), &mask);
}
...
{
	return nr_free_zone_pages_bs(gfp_zone(GFP_USER));
}
...
{
	return nr_free_zone_pages_bs(gfp_zone(GFP_HIGHUSER));
}
